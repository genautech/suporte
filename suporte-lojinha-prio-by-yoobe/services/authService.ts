 ...
 */
export const validateAuthCode = async (email: string, code: string): Promise<boolean> => {
  const normalizedEmail = email.toLowerCase().trim();
...
/**
 * Sends authentication code email via Postmark proxy
 */
export const sendAuthCodeEmail = async (email: string, code: string): Promise<{ success: boolean; error?: string }> => {
  ...
  try {
    ...
    console.error("Erro ao enviar e-mail de autenticação via proxy:", error);
