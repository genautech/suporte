...
```ts
import { useCreateUser, useListFaQs, useCreateMessage, useGetConversationsForUser } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useCreateUser();

const { data, isPending, isSuccess, isError, error } = useListFaQs();

const { data, isPending, isSuccess, isError, error } = useCreateMessage(createMessageVars);

const { data, isPending, isSuccess, isError, error } = useGetConversationsForUser();
